...
class ViewController: UIViewController, PrintInstructionProtocol {
    ...
    lazy var searchButton: UIButton = {
        ...
        temporaryButton.titleEdgeInsets = UIEdgeInsets(top: 0, left: -20, bottom: 0, right: 0)
        temporaryButton.setTitle("搜索中", for: .normal)
        temporaryButton.setTitle("停止搜索", for: .selected)
        temporaryButton.setTitleColor(UIColor.red, for: .normal)
    ...
    lazy var printButton: UIButton = {
        let temporaryButton = UIButton(type: .custom)
        temporaryButton.bounds = CGRect(x: 0, y: 0, width: 80, height: 50)
        temporaryButton.titleEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: -30)
        temporaryButton.setTitle("打印", for: .normal)
        temporaryButton.setTitleColor(UIColor.red, for: .normal)
        temporaryButton.setTitleColor(UIColor.red, for: .selected)
        return  temporaryButton
    }()
    ...
    @objc func print() {
        ...
        let printTemplate = PrintTemplate(storeName: "XXXX(xxxx)", number: "A001", tableType: TableType.tableForTwo, waiting: "您前面还有:0桌在等候", time: "排队时间:03月26日10时10分", phone: "餐厅电话:13696888888", qrcode: "https://mp.weixin.qq.com/cgi-bin/showqrcode?ticket=gQHj8TwAAAAAAAAAAS5odHRwOi8vd2VpeGluLnFxLmNvbS9xLzAyNEtOU003WmQ4OV8xcTY5NTFxMU8AAgTG4ERaAwTAqAAA")
        for peripheral in printerArrary! {
            if peripheral.state == .connected {
                ...
                //bluetoothManager.writeValue(peripheral, data: printerInitialize)
                //bluetoothManager.writeValue(peripheral, data: printerLeftSpacing(50, nH: 0))
                //bluetoothManager.writeValue(peripheral, data: printerQRCode(12, ecc: 48, qrcode: printTemplate.qrcode))
                //bluetoothManager.writeValue(peripheral, data: printer(paperFeed: 1))
